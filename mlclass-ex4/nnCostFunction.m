...
% Mayer: Adding Bias Units to the Input Layer
X = [ones(m,1) X];
...
% Mayer: Feedforward Propagation for each training example
a1 = X;
...
% Mayer: Reshape y to a m * num_classes matrice
y1 = eye(num_labels)(y,:);
J = -(1/m) * sum( sum(y1 .* log(a3) + (1-y1).*log(1-a3)) );


% Mayer: Build Regularization Parameter
...
reg1 = [zeros(hidden_layer_size,1) reg1];
reg2 = [zeros(num_labels,1) reg2];
